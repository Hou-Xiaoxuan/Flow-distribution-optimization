...
class SA {
    ...
    //[edge_site] = flow_94
    vector<int> flow_89_per_edge_site;

    //[edge_site] = flow_95
    vector<int> flow_90_per_edge_site;

    //[edge_site] = flow_96
    vector<int> flow_91_per_edge_site;
...
public:
    ...
    inline int get_k89_order() {
        return max((data.get_mtime_num() * 18 - 1) / 20 + 1 - 1, 1ul);
    }

    inline int get_k90_order() {
        return (data.get_mtime_num() * 18 - 1) / 20 + 1;
    }

    inline int get_k91_order() {
        return min((data.get_mtime_num() * 18 - 1) / 20 + 1 + 1, data.get_mtime_num());
    }
    ...
    void simple_ffd() {
        ...
        flow_89_per_edge_site.assign(data.get_edge_num(), 0);
        flow_90_per_edge_site.assign(data.get_edge_num(), 0);
        flow_91_per_edge_site.assign(data.get_edge_num(), 0);

        edge_site_90.assign(data.get_edge_num(), 0);
        for (size_t edge_site = 0; edge_site < data.get_edge_num(); ++edge_site) {
            edge_site_90[edge_site] = edge_site;
        }
        ...
        index_89 = get_k89_order();
        index_90 = get_k90_order();
        index_91 = get_k91_order();
...
#endif
            T = T * dT; //降温
            if (fabs(last_cost - now_cost) < 1e-5) {
                if (++now_change_cnt > 1000) {
                    break;
                }
            } else {
                now_change_cnt = 0;
    ...
    void SA_decent_90(double T, double dT, double end_T) {
        ...
        for (size_t edge_index = 0; edge_index < 10; ++edge_index) {
            ...
            if (data.get_mtime_num() > 1) {
                flow_89_per_edge_site[edge_site] =
                    tree[edge_site].queryK(0, 0, data.site_bandwidth[edge_site], index_89);
            }
            if (data.get_mtime_num() >= 19) {
                flow_91_per_edge_site[edge_site] =
                    tree[edge_site].queryK(0, 0, data.site_bandwidth[edge_site], index_91);
            }
            int flow_90 = tree[edge_site].queryK(0, 0, data.site_bandwidth[edge_site], index_90);
            flow_90_per_edge_site[edge_site] = flow_90;

            auto it = find(edge_site_95_has_flow.begin(), edge_site_95_has_flow.end(), edge_site);
            if (it != edge_site_95_has_flow.end()) {
                edge_site_95_has_flow.erase(it);
            }
            ...
            if (total_flow_per_edge_site[edge_site] == 0) {
                ...
                if (flow_90 <= data.base_cost) {
                    cost = data.base_cost;
                } else {
                    cost =
                        1.0 * (flow_90 - data.base_cost) * (flow_90 - data.base_cost) / data.site_bandwidth[edge_site] +
                        flow_90;
                }
            }
            now_cost += cost - cost_per_edge_site[edge_site];
            cost_per_edge_site[edge_site] = cost;
        }
        ...
        while (T > end_T) {
            ...
            bool flag_one = false;
            if (find(edge_site_90.begin(), edge_site_90.begin() + 10, edge_site_one) != edge_site_90.begin() + 10) {
                flag_one = true;
            }
            ...
            int m_time = 0;
            if (flag_one == false) {
                m_time = *flow_to_m_time_per_edge_site[edge_site_one][flow_95_per_edge_site[edge_site_one]].begin();
            } else {
                m_time = *flow_to_m_time_per_edge_site[edge_site_one][flow_90_per_edge_site[edge_site_one]].begin();
            }
            ...
            bool flag_two = false;
            if (find(edge_site_90.begin(), edge_site_90.begin() + 10, edge_site_two) != edge_site_90.begin() + 10) {
                flag_two = true;
            }
            ...
            if (flag_one == false) {
                ...
                if (data.get_mtime_num() == 1) { // 特殊情况只有一个时点
                    ...
                    if (new_flow_one >= flow_89_per_edge_site[edge_site_one]) {
                        new_flow_95_one = new_flow_one;
                    } else {
                        new_flow_95_one = flow_89_per_edge_site[edge_site_one];
                    }
                }
            }
            ...
            /* 确定new_flow_95_two值是多少*/
            int old_flow_95_two = 0;
            int new_flow_95_two = 0;
            if (flag_two == false) {
                old_flow_95_two = flow_95_per_edge_site[edge_site_two];
                if (data.get_mtime_num() < 20) { // 特殊情况时点小于20
                    if (new_flow_two > flow_95_per_edge_site[edge_site_two]) {
                        new_flow_95_two = new_flow_two;
                    } else {
                ...
                old_flow_95_two = flow_90_per_edge_site[edge_site_two];
                if (data.get_mtime_num() < 19) { // 特殊情况时点小于20
                    if (new_flow_two > flow_90_per_edge_site[edge_site_two]) {
                        new_flow_95_two = new_flow_two;
                    } else {
                        new_flow_95_two = flow_90_per_edge_site[edge_site_two];
                    }
                } else {
                    if (old_flow_two > flow_90_per_edge_site[edge_site_two]) {
                        new_flow_95_two = flow_90_per_edge_site[edge_site_two];
                    } else {
                        if (new_flow_two > flow_91_per_edge_site[edge_site_two]) {
                            new_flow_95_two = flow_91_per_edge_site[edge_site_two];
                        } else if (new_flow_two > flow_90_per_edge_site[edge_site_two]) {
                            new_flow_95_two = new_flow_two;
                        } else {
                            new_flow_95_two = flow_90_per_edge_site[edge_site_two];
                        }
            ...
            if (is_acc == true) {
                ...
                if (flag_two == false) {
                    ...
                    if (data.get_mtime_num() < 19) { // 特殊情况时点小于20 不需要维护96值
                        if (old_flow_two == flow_90_per_edge_site[edge_site_two]) {
                            flow_90_per_edge_site[edge_site_two] = new_flow_two;
                        } else if (old_flow_two <= flow_89_per_edge_site[edge_site_two]) {
                            if (new_flow_two > flow_90_per_edge_site[edge_site_two]) {
                                if (data.get_mtime_num() > 1) {
                                    flow_89_per_edge_site[edge_site_two] = flow_90_per_edge_site[edge_site_two];
                                }
                                flow_90_per_edge_site[edge_site_two] = new_flow_two;
                            } else if (new_flow_two > flow_89_per_edge_site[edge_site_two]) {
                                if (data.get_mtime_num() > 1) {
                                    flow_89_per_edge_site[edge_site_two] = new_flow_two;
                                }
                            }
                        }
                    } else {
                        if (old_flow_two >= flow_91_per_edge_site[edge_site_two]) {
                            if (old_flow_two == flow_91_per_edge_site[edge_site_two]) {
                                flow_91_per_edge_site[edge_site_two] =
                                    tree[edge_site_two].queryK(0, 0, data.site_bandwidth[edge_site_two], index_91);
                            }
                        } else if (old_flow_two == flow_90_per_edge_site[edge_site_two]) {
                            if (new_flow_two > flow_91_per_edge_site[edge_site_two]) {
                                flow_90_per_edge_site[edge_site_two] = flow_91_per_edge_site[edge_site_two];
                                flow_91_per_edge_site[edge_site_two] =
                                    tree[edge_site_two].queryK(0, 0, data.site_bandwidth[edge_site_two], index_91);
                            } else if (new_flow_two > flow_90_per_edge_site[edge_site_two]) {
                                flow_90_per_edge_site[edge_site_two] = new_flow_two;
                ...
                if (flag_one == false) {
                    ...
                    if (data.get_mtime_num() == 1) { // 特殊情况只有一个时点
                        flow_90_per_edge_site[edge_site_one] = new_flow_one;
                    } else {
                        if (new_flow_one >= flow_89_per_edge_site[edge_site_one]) {
                            flow_90_per_edge_site[edge_site_one] = new_flow_one;
                        } else {
                            flow_90_per_edge_site[edge_site_one] = flow_89_per_edge_site[edge_site_one];
                            flow_89_per_edge_site[edge_site_one] =
                                tree[edge_site_one].queryK(0, 0, data.site_bandwidth[edge_site_one], index_89);
                        }
                    }
                }
        ...
        for (size_t edge_index = 0; edge_index < 10; ++edge_index) {
            int edge_site = edge_site_90[edge_index];
            flow_94_per_edge_site[edge_site] = tree[edge_site].queryK(0, 0, data.site_bandwidth[edge_site], index_94);
            flow_95_per_edge_site[edge_site] = tree[edge_site].queryK(0, 0, data.site_bandwidth[edge_site], index_95);
            flow_96_per_edge_site[edge_site] = tree[edge_site].queryK(0, 0, data.site_bandwidth[edge_site], index_96);
        }
    ...
    Distribution excute() {
        ...
        // SA_decent(1e15, 0.99, 1e-15);
        SA_decent(1e15, 0.999994, 1e-15);
        SA_shuffle(1e20, 0.99, 1e17);
        SA_decent(1e5, 0.999999, 1e-20);
        cout << "shuffle and decent over" << endl << endl;

        SA_shuffle(1e30, 0.99, 1e25);
        SA_decent(1e5, 0.999999, 1e-20);
        cout << "shuffle and decent over" << endl << endl;
        ...
        vector<pair<int, int>> tmp(data.get_edge_num());
        ...
        int index = 0;
        edge_site_90.resize(0);
        for (size_t edge_site = 0; edge_site < data.get_edge_num(); ++edge_site) {
            edge_site_90.push_back(tmp[edge_site].second);
            if (tmp[edge_site].first != 0) {
                index = edge_site;
            }
        }
