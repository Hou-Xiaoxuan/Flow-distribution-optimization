 ...
 * @Description:
 * @LastEditors: LinXuan
 * @LastEditTime: 2022-04-02 17:18:46
...
private:
    ...
    /* 在m_time上执行匹配 */
    bool excute_match_per_mtime(vector<Stream> &stream_node, vector<vector<int>> &matched_per_edge, vector<int> &residual_capacity) {
    ...
    void update_distribution_per_mtime(size_t mtime, vector<vector<int>> &matched_per_edge, vector<Stream> &stream_node) {
        // 写入distribution
...
public:
    ...
    Distribution excute() {
        ...
        for (size_t mtime = 0; mtime < data.get_mtime_num(); mtime++) {
            ...
            this->excute_match_per_mtime(stream_node, matched_per_edge, residual_capacity);
